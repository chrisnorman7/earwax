...
class Box(EventDispatcher):
    ...
    :ivar ~earwax.Box.surface_generator: The ``synthizer.BufferGenerator`` that
        will play surface sounds on this instance.
    """
    ...
    surface_generator: Optional[BufferGenerator] = None
    ...
    def on_footstep(self) -> None:
        ...
        if self.surface_sound is not None and self.context is not None:
            play_path(
                self.context, self.surface_sound,
                generator=self.surface_generator, source=self.source
            )
